describe("My First Test", () => {
  ...
  it("status select should working as expected", () => {
    ...
    cy.getByTestId('main-list-item-name').eq(0).should('have.text', 'Adjudicator Rick')
    cy.getByTestId('main-list-item-name').eq(3).should('have.text', 'Albert Einstein')
  ...
  it("gender select should working as expected", () => {
    ...
    cy.getByTestId('main-list-item-name').eq(0).should('have.text', 'Alien Googah')
    cy.getByTestId('main-list-item-name').eq(3).should('have.text', 'Blamph')
    cy.getByTestId('main-list-item-name').eq(9).should('have.text', 'Dipper and Mabel Mortys')
  });
  ...
  it("search input should working as expected", () => {
    ...
    cy.wait('@apiRequest');
    cy.getByTestId('main-list-item-name').eq(0).should('have.text', 'Summer Smith')
    cy.getByTestId('main-list-item-name').eq(2).should('have.text', 'Mechanical Summer')
    cy.getByTestId('main-list-search-filter-input').clear()
    cy.getByTestId('main-list-search-filter-input').type("100% without results");
    cy.wait('@apiRequest');
    cy.getByTestId('main-list-item-link').should('have.length', 0)
    cy.getByTestId('main-list-search-filter-input').clear()
    cy.getByTestId('main-list-search-filter-input').type("morty");
    cy.wait('@apiRequest');
    cy.getByTestId('main-list-item-name').eq(0).should('have.text', 'Morty Smith')
    cy.getByTestId('main-list-item-name').eq(9).should('have.text', 'Campaign Manager Morty')
  });
});
