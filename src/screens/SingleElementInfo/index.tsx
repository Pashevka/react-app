...
export const SingleElementInfo: React.FC = () => {
  ...
  const element = useSelector<RootState, IListElement | null>(
    state => state.singleElementReducer.element
  );
  const hasErrors = useSelector<RootState, boolean>(
    state => state.singleElementReducer.hasErrors
  );
  ...
  const onChangeData = (key: keyof IListElement, value: string) => {
    dispatch(
      singleElementSliceActions.setNewData({
        key: key,
        value: value,
      })
    );
